...
func TestOneSite(t *testing.T) {
	chain, _ := certificateUtils.GatherCertificateChain("https://valid-certum-ctnca.certificates.certum.pl/")
	b, _ := pem.Decode([]byte(root))
	anchor, _ := x509.ParseCertificate(b.Bytes)
	chain = certificateUtils.EmplaceRoot(chain, anchor)
	for _, cert := range chain {
		t.Log(certificateUtils.FingerprintOf(cert))
		p := pem.Block{"CERTIFICATE", nil, []byte(base64.StdEncoding.EncodeToString(cert.Raw))}
	...
	roots.AddCert(chain[3])
	ints.AddCert(chain[1])
	ints.AddCert(chain[2])
	opts := x509.VerifyOptions{
		Roots:         roots,
		Intermediates: ints,
	}
	answer, err := chain[0].Verify(opts)
	if err != nil {
		t.Log(answer)
		t.Fatal(err)
	}
